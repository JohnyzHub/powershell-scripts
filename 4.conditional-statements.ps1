...
Clear-Host
if (4 -eq 4) {
    Write-Host "[4 -eq 4]? 4 is equal to 4"
}

Clear-Host
if ("hello" -eq "hello") {
    Write-Host "[hello -eq hello]? Both strings are equal."
} 
...
[int]$y = 10
if ($x -eq $y) {
    Write-Host "[$x -eq $y]? yes"
}
else {
    Write-Host "[$x -eq $y]? No"
}
...
$yourName = "Ian"
if ($yourName -eq "Ian") {
    Write-Host "[$yourName -eq Ian]? Hay my name is Ian too!"
}
else {
    Write-Host "[$yourName -eq Ian]? Hi $yourName, nice to meet you!"
}
...
# -eq	Equals.	
if (5 -eq 5) {
    Write-Host "[5 -eq 5]? Yes"
}
   
# -ne Not equals.
Clear-Host
if (3 -ne 4) {
    Write-Host "[3 -ne 4]? No"
}
   
# -gt Greater than.
Clear-Host
if (4 -gt 2) {
    Write-Host "[4 -gt 2]? Yes"
}
   
# -ge Greater than or equal.
Clear-Host
if (2 -ge 1) {
    Write-Host "[2 -ge 1]? 2 is greater than or equal to 1"
}
   
# -lt Less than.
Clear-Host
if (1 -lt 2) {
    Write-Host "[1 -lt 2]? 1 is less than 1"
}

# -le Less than or equal.
Clear-Host
if (1 -le 2) {
    Write-Host "[1 -le 2]? 1 is less than or equal to 2"
} 
...
if ("hello how are you?" -like "hello*") {
    Write-Host "[hello how are you? -like hello*]? Yes."
}

# -notlike	Results in a true condition when two strings don't match.	
Clear-Host
if ("HELLO" -notlike "BYE") {
    Write-Host "[HELLO -notlike BYE] ? Yes, both are different."
} 
...
Clear-Host
if ("HELLO" -match "H") {
    Write-Host "[HELLO -match H]? H exists in HELLO"
}
...
if ("HELLO" -notmatch "A") {
    ...
    Write-Host "[HELLO -notmatch A]? A doesn't exists in HELLO"
}
...
if ($list -contains 3) {
    ...
    Write-Host "[$list] -contains 3? yes."
}
...
if ($list -notcontains 8) {
    Write-Host "[$list] -notcontains 8 ? 8 is in the list."
}
...
$list = @(1, 2, 3, 4, 5)
if (3 -in $list) {
    Write-Host "3 -in [$list] ? 3 is in the list."
} 
...
$list = @(1, 2, 3, 4, 5)
if (6 -notin $list) {
    Write-Host "6 -notin [$list] ? 6 is not in the list."
}
...
Clear-Host
[string]$favouriteColour = Read-Host "Pick one color[Red, Blue]?"
